...
% Generate 500 uniform random variables on the interval [0, 1]
U1 = rand(500, 1);
U2 = rand(500, 1);

% Transform the uniform random variables into standard Gaussian random variables
Z1 = sqrt(-2 * log(U1)) .* cos(2 * pi * U2);
Z2 = sqrt(-2 * log(U1)) .* sin(2 * pi * U2);

% Transform the standard Gaussian random variables into correlated Gaussian random variables
L = chol([1 0.5; 0.5 2], 'lower');
XY = L * [Z1'; Z2'];

% Add the generated X and Y values to the mean vector [0 0] to obtain the final sample data
x = XY(1, :)';
y = XY(2, :)';
z = exp(-0.5*((x - 0.5).^2/2 + (y - 0.5).^2/5)/1^2); % Evaluate the function at each (x, y) point

data = [x, y, z];

%% run the gradient descent algorithm
% Define the cost function
costFunc = @(params) sum((data(:, 3) - gaussian2d(params, data(:, 1), data(:, 2))).^2);

% Initial guess for the parameters
params0 = [0.5, 0.5, 0.1, 0.1, 0.1];
...
for i = 1:nIter
    ...
    grad = numerical_gradient_n_dim(@(params) costFunc(params), params, 1e-6);
